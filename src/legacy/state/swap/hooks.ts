...
import { Currency, CurrencyAmount, AMBER, JSBI, Token, TokenAmount, Trade } from '@firepotfinance/firepotfinance-sdk';
  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency === AMBER ? 'AMB' : '',
        })
      );
    },
    [dispatch]
...
function parseCurrencyFromURLParameter(urlParam: any): string {
  if (typeof urlParam === 'string') {
    ...
    if (urlParam.toUpperCase() === 'AMB') return 'AMB';
    if (valid === false) return 'AMB';
  }
  return 'AMB' ?? '';
}
